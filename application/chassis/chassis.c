 ...
 *        用 DWT 时间轴驱动,与任务调用频率解耦,改频率也不影响节奏
 */
static float SpinSpeed(void)
{
    float t = DWT_GetTimeline_ms() / 1000.0f; // 连续时间(s)
    return SPIN_WZ_BASE + SPIN_WZ_AMP * arm_sin_f32(2.0f * PI * SPIN_WZ_FREQ * t);
}
...
void ChassisTask()
{
    ...
    case CHASSIS_ROTATE_CLOCKWISE: // 变速小陀螺(顺时针),转速正弦波动降低被命中概率;800大概100rpm 再大会吞线速度
        chassis_cmd_recv.wz = -SpinSpeed();
        break;
    case CHASSIS_ROTATE_COUNTERCLOCKWISE: // 变速小陀螺(逆时针)
        chassis_cmd_recv.wz = SpinSpeed();
        break;
    default:
        break;

...
static volatile float chassis_power_limit_fixed = 25.0f;
...
/* Buffer环暂未测试，逻辑是计算期望buffer与实际buffer的差值，转换为冗余的功率，todo：输入给功率控制部分，待完善 */
    ...
    // PID controller for chassis follow gimbal yaw in CHASSIS_FOLLOW_GIMBAL_YAW mode
    PID_Init_Config_s follow_pid_conf = {
        .Kp = 5.0f,
        .Ki = 0.0f,
        .Kd = 0.0f,
        .DeadBand = 3.0f,
    ...
    PIDInit(&chassis_follow_PID, &follow_pid_conf);
...
void ChassisTask()
{
    ...
    // 老步兵裁判系统(电源管理)未接线,referee_data->chassis_power_limit 恒为0会把功率压成0导致底盘不动
    // 此处用 chassis_power_limit_fixed 替代,可在Ozone中实时修改调参; 接回裁判系统时改回下面注释那行
    // SetPowerLimit(referee_data->GameRobotState.chassis_power_limit);
    SetPowerLimit(chassis_power_limit_fixed); // 设置功率限制(可调变量)
    ...
    case CHASSIS_ROTATE: // 自旋,同时保持全向机动;当前wz维持定值,后续增加不规则的变速策略
        ...
        break;
    case CHASSIS_ROTATE_INVERSE: // 自旋,同时保持全向机动;当前wz维持定值,后续增加不规则的变速策略
        chassis_cmd_recv.wz = -1000;
        break;
    default:
        break;

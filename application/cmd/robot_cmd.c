...
static uint8_t qe_spin_mode = 0;        // Q/E切换式小陀螺状态: 0=跟随云台,1=逆时针(Q),2=顺时针(E); 急停/遥控器模式会清零
 ...
 */
static uint8_t VisionAimGimbal()
{
...
/**
 * @brief 遥控器右摇杆手动控制云台(yaw/pitch),供左拨杆下位手动模式与视觉无目标回退共用
 */
static void RemoteControlGimbalManual()
{
...
{
    qe_spin_mode = 0; // 遥控器模式不使用键盘小陀螺开关,清零以便切回键鼠时从跟随态开始
    ...
    // 视觉模式下若无目标(VisionAimGimbal返回0),回退到右摇杆手动控制,以便操作手仍能搜索/转向
    if (switch_is_mid(rc_data[TEMP].rc.switch_left))
    {
        if (!VisionAimGimbal())
            RemoteControlGimbalManual();
    }
    else
    {
        RemoteControlGimbalManual();
...
/**
 * @brief 鼠标/键盘手动控制云台(yaw/pitch + C键一键掉头),供普通键鼠模式与视觉无目标回退共用
 */
static void MouseKeyGimbalManual()
{
...
static void MouseKeySet()
{
    ...
    // Q/E一键开关小陀螺(切换式): 按下Q进左旋,再按一下退出; 按下E进右旋,再按一下退出; 按相反方向键直接切换转向
    // qe_spin_mode为模块级变量,急停/切到遥控器模式时会被清零
    static uint8_t q_key_last = 0, e_key_last = 0;
    uint8_t q_key_now = rc_data[TEMP].key[KEY_PRESS].q;
    uint8_t e_key_now = rc_data[TEMP].key[KEY_PRESS].e;
    if (q_key_now && !q_key_last) // Q上升沿: 切换左旋
        qe_spin_mode = (qe_spin_mode == 1) ? 0 : 1;
    if (e_key_now && !e_key_last) // E上升沿: 切换右旋
        qe_spin_mode = (qe_spin_mode == 2) ? 0 : 2;
    q_key_last = q_key_now;
    e_key_last = e_key_now;
    ...
    if (vision_mode)
    {
        ...
        // 无目标时(VisionAimGimbal返回0)回退到鼠标手动控制,以便操作手仍能搜索/转向
        if (!VisionAimGimbal())
            MouseKeyGimbalManual();
        // 视觉模式发射:
...
static void EmergencyHandler()
{
    ...
    if (rc_offline || rc_data[TEMP].rc.dial > 300 || kb_stop || robot_state == ROBOT_STOP)
    {
        ...
        qe_spin_mode = 0;                             // 急停清零小陀螺开关,恢复后不自动续转
